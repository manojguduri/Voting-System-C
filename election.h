...
struct currentValidID{
    int year;
    char branch[5]; // To accommodate the branch code format
    int totalVoters;
};
...
//To extract year from userID -- For example, userID:VU21EECE0100076 year:2021 
int extractYear(char userID[15])
{
    int year=0;
    char tmp;
    for(int i=2;i<4;i++){ // Extracting the year from the 3rd and 4th characters
        tmp=userID[i];
        year=(year*10)+(tmp-48);
    }
    return 2000 + year; // Assuming that the year representation is in 21st century
}
...
int extractRollNo(char userID[15])
{
    int rollno=0;
    char tmp;
    for(int i=9;i<15;i++){ //index for roll number extraction
        tmp=userID[i];
...
{
    char branchCode[5];
    for(int i=4;i<8;i++){ // Extracting the branch code from the 5th to 8th characters
        branchCode[i-4]=userID[i];
    }
    branchCode[4]='\0'; // Adjusted the length of the branch code string
...
int authenticateAdmin(){
    ...
    else
    {
        ...
        password[i]='\0';
        if((strcmp(password,"Gitam$$123"))!=0)
            return 0;
    }
    return 1;
}
